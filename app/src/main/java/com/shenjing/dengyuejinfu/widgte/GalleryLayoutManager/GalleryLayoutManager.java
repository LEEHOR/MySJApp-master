package com.shenjing.dengyuejinfu.widgte.GalleryLayoutManager;
...
public class GalleryLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private LinearSnapHelper mSnapHelper = new LinearSnapHelper();  //可实现居中
//    private CustomSnapHelper mSnapHelper = new CustomSnapHelper();
            ...
            if (dy >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedBottom(child) - dy < topEdge) {
                        if (BuildConfig.DEBUG) {
                            LogUtils.v(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedTop(child) - dy > bottomEdge) {
                        if (BuildConfig.DEBUG) {
                            LogUtils.v(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child));
                        }
                        removeAndRecycleView(child, recycler);
                        mLastVisiblePos--;
                    } else {
    ...
    private void fillWithHorizontal(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        ...
        int rightEdge = getOrientationHelper().getEndAfterPadding();
        if (BuildConfig.DEBUG) {
            LogUtils.v(TAG, "fillWithHorizontal() called with: dx = [" + dx + "],leftEdge:" + leftEdge + ",rightEdge:" + rightEdge);
        }
        //1.remove and recycle the view that disappear in screen
        View child;
        if (getChildCount() > 0) {
            if (dx >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedRight(child) - dx < leftEdge) {
                        ...
                        fixIndex--;
                        if (BuildConfig.DEBUG) {
                            LogUtils.v(TAG, "fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + " mFirstVisiblePosition change to:" + mFirstVisiblePosition);
                        }
                    } else {
                        break;
                    }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedLeft(child) - dx > rightEdge) {
                        ...
                        mLastVisiblePos--;
                        if (BuildConfig.DEBUG) {
                            LogUtils.v(TAG, "fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + "mLastVisiblePos change to:" + mLastVisiblePos);
                        }
                    }
                }
            }
    ...
    public void attach(RecyclerView recyclerView, int selectedPosition) {
        ...
        RecyclerView.OnFlingListener onFlingListener = new RecyclerView.OnFlingListener() {
            ...
            public boolean onFling(final int velocityX, final int velocityY) {
                LogUtils.e(TAG, "onFling: onTouchEvent 1 mState=" + mState);
                return false;
            }
        };
    ...
    private class InnerScrollListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (snap != null) {
                ...
                if (selectedPosition != mCurSelectedPosition) {
                    ...
                    if (!mCallbackInFling && mState != SCROLL_STATE_IDLE) {
                        if (BuildConfig.DEBUG) {
                            LogUtils.v(TAG, "ignore selection change callback when fling ");
                        }
            ...
            if (BuildConfig.DEBUG) {
                LogUtils.v(TAG, "onScrolled: dx:" + dx + ",dy:" + dy);
            }
        }
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            mState = newState;
            if (BuildConfig.DEBUG) {
                LogUtils.v(TAG, "onScrollStateChanged: " + newState);
            }
