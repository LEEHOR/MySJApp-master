...
import com.shenjing.mytextapp.ui.contract.RegisterActivityContract;
import com.shenjing.mytextapp.ui.presenter.RegisterActivityPresenter;
...
//        userRegisterBean.setFacilityCode(imei);
    ...
    @Override
    public void registerSuccess(RegisterModule registerModule) {
        Map map = new HashMap();
        String md5_pass = EncryptUtils.encryptMD5ToString(registerUserLoginPass.getText().toString().trim());
        map.put("password", md5_pass);
        map.put("phoneNumber", registerUserPhone.getText().toString().trim());
...
//        accountLoginBean.setPassword(registerUserLoginPass.getText().toString().trim());
    ...
    @Override
    public void loginSuccess(LoginModule loginModule) {
        spUtils.put(BaseParams.USER_NAME_KEY, loginModule.getData().getName(), true);
        spUtils.put(BaseParams.USER_TOKEN_KEY, loginModule.getData().getToken(), true);
        spUtils.put(BaseParams.USER_ID_KEY, loginModule.getData().getId(), false);
        BaseParams.userName = loginModule.getData().getName();
        BaseParams.userId = loginModule.getData().getId();
        BaseParams.userToken = loginModule.getData().getToken();
